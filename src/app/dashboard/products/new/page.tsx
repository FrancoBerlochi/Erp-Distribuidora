...
import { ArrowLeft, Upload, X, Barcode, Camera, Sparkles, AlertTriangle, CheckCircle2, Percent } from 'lucide-react';
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (images.length + selectedFiles.length > 2) {
        alert('Solo puedes subir un máximo de 2 imágenes por producto.');
        return;
      }
    ...
    try {
      ...
      if (images.length > 0) {
        if (!process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME) {
            throw new Error("Cloudinary no está configurado aún.");
        }
        image_url_1 = await uploadToCloudinary(images[0]);
        if (images.length > 1) {
          image_url_2 = await uploadToCloudinary(images[1]);
        }
      }
      ...
      if (formData.cost_price && formData.cost_price.trim() !== '') {
        productPayload.cost_price = parseFloat(formData.cost_price);
      }

      if (formData.wholesale_price && formData.wholesale_price.trim() !== '') {
        productPayload.wholesale_price = parseFloat(formData.wholesale_price);
        productPayload.wholesale_min_qty = parseInt(formData.wholesale_min_qty) || 6;
      }

      if (formData.promo_type && formData.promo_type !== 'none') {
        productPayload.promo_type = formData.promo_type;
        if (formData.promo_type === 'second_unit_discount') {
          productPayload.promo_second_unit_discount = parseFloat(formData.promo_second_unit_discount) || 70;
        } else if (formData.promo_type === 'volume_discount') {
          productPayload.promo_min_qty = parseInt(formData.promo_min_qty) || 3;
          productPayload.promo_discount_percentage = parseFloat(formData.promo_discount_percentage) || 15;
        }
      }

      // Insertar en Supabase (con fallback defensivo ante columnas pendientes)
      let { error } = await supabase.from('products').insert([productPayload]);
      if (error && error.code === '42703') {
        delete productPayload.min_stock;
        delete productPayload.supplier_name;
        delete productPayload.promo_type;
        delete productPayload.promo_second_unit_discount;
        delete productPayload.promo_min_qty;
        delete productPayload.promo_discount_percentage;
        delete productPayload.wholesale_price;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">Nuevo Producto</h1>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-900 dark:text-gray-100 mb-1.5">Stock Inicial *</label>
              <input 
                required 
                type="number" 
                min="0"
            ...
            <div className="col-span-2 border-t border-gray-200 dark:border-gray-800 pt-6 mt-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Imágenes (Máx 2)</label>
              
              <div className="flex gap-4 mb-4">
                {imagePreviewUrls.map((url, i) => (
                  <div key={i} className="relative w-24 h-24 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-800">
                ...
                {images.length < 2 && (
                  <label className="w-24 h-24 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer hover:text-primary hover:border-primary transition-colors">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Puedes subir hasta 2 imágenes por producto. Recomendado: formato 1:1 (cuadrado).</p>
            </div>
          </div>
